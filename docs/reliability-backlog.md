...
| sandbox `inherit` 模式 | PARTIAL | `resolveSandboxMode`（`codex.mjs:67-79`）永遠回具體 mode,從不 null | 加 `inherit` sentinel:回 null 時 `buildThreadParams`/`buildResumeParams` 省略 sandbox 鍵,讓 Codex 用自己的 config。PR #241 |
